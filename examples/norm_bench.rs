...
fn main() {
    ...
    for line in reader.lines() {
        ...
        if fn_only && outcome != "FN" { continue; }
        ...
        println!("{label}  score={:.2}  [{:<30}]  {:?}",
            r.obfuscation_score,
            kinds.join(","),
            norm_preview
        );
        if !detected { missed.push(text[..text.len().min(80)].to_string()); }
    }

    println!();
    println!("──────────────────────────────────────────────");
    println!("Caught:  {caught}/{total}  ({:.0}%)", caught as f64 / total.max(1) as f64 * 100.0);
